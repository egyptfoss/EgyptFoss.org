...
class NewsletterEmails extends NewsletterModule {
    ...
    function __construct() {
        ...
        if (is_admin()) {
            ...
            add_action('wp_ajax_tnpc_render_ar', array($this, 'tnpc_render_ar_callback'));
            add_action('wp_ajax_tnpc_preview_ar', array($this, 'tnpc_preview_ar_callback'));
    ...
    function tnpc_render_ar_callback() {
        $block_options = get_option('newsletter_main');
        $block = $this->get_block($_POST['b']);
        if ($block) {
    ...
    function tnpc_preview_ar_callback() {
        $email = Newsletter::instance()->get_email($_REQUEST['id'], ARRAY_A);
        ...
        echo get_option( 'newsletter_' . $email['id'] . '_ar_message' );
