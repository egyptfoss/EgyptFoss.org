...
if ($controls->is_action('theme')) {
    ...
    if ($controls->data['theme'] == 'rawhtml' || !file_exists($module->get_current_theme_file_path('theme-options.php'))) {
        ...
        if ($controls->data['theme'] == 'rawhtml') {
            ...
            $ar_email['message'] = "<!DOCTYPE html>\n<html>\n<head>\n<title>عنوان قائمتك البريدية</title>\n</head>\n<body>\n</body>\n</html>";
        } else {
            ...
            $ar_email['message'] = $email['message'];
        ...
        $ar_email['subject'] = $email['subject'];
        $email['type'] = 'message';
        $email['send_on'] = time();
        $email = Newsletter::instance()->save_email($email, OBJECT);
        
        $email_id = $wpdb->insert_id;
        
        if( $ar_email['subject'] ) {
          update_option('newsletter_' . $email_id . '_ar_subject', $ar_email['subject'] );
        }

        if( $ar_email['message'] ) {
          update_option('newsletter_' . $email_id . '_ar_message', $ar_email['message'] );
        }
        ?>
        <script>
