...
//        }
    ...
    function hook_init() {
        ...
        if ($action == 'fu') {
            ...
            $unsubscribed_text = (pll_current_language() == 'en' )?$options_followup['unsubscribed_text']:$options_followup['unsubscribed_text_ar'];
            $this->message = $unsubscribed_text;
            return;
        }
...
//            if ($skip_this_run) return false;
        ...
        foreach ($users as $user) {
            ...
            if( $wp_user ) {
              $language = get_user_meta( $user->id, 'prefered_language', true );
            } else {
              if( strpos( $user->http_referer, 'ar' ) !== false ) {
                $language = 'ar';
              }
            }
            
            $ar_message = get_option( 'newsletter_' . $email->id . '_ar_message' );
            $ar_subject = get_option( 'newsletter_' . $email->id . '_ar_subject' );
            
            if( $language == 'ar' && $ar_message ) {
              $m = $this->replace($ar_message, $user, $email->id);
            }
            else {
            ...
            if( $language == 'ar' && $ar_subject ) {
                $s = $this->replace($ar_subject, $user);
            }
            else {
