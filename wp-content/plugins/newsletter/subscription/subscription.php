...
class NewsletterSubscription extends NewsletterModule {
    ...
    function hook_wp_enqueue_scripts() {
        ...
        if (isset($options['email_error'])) {
            $email_error = ( pll_current_language() == 'en' )?$options['email_error']:$options['email_error_ar'];
            $data['messages']['email_error'] = $email_error;
        }
        ...
        if (isset($options['profile_error'])) {
            $profile_error = ( pll_current_language() == 'en' )?$options['profile_error']:$options['profile_error_ar'];
            $data['messages']['profile_error'] = $profile_error;
        }
            ...
            if ($status != 'C' && $opt_in == 0) {
                ...
                if ($user->status == 'C') {
                    ...
                    if (empty($options[$prefix . 'disabled'])) {
                        ...
                        $message = ( pll_current_language() == 'en' )?$options['confirmation_message']:$options['confirmation-ar_message'];
                        $subject = ( pll_current_language() == 'en' )?$options['confirmation_subject']:$options['confirmation-ar_subject'];
...
//                return $user;
        ...
        if (empty($options[$prefix . 'disabled'])) {
            if( pll_current_language() == 'ar' ) {
              $prefix = ($user->status == 'C') ? 'confirmed-ar_' : 'confirmation-ar_';
            }
    ...
    function confirm($user_id = null, $emails = true) {
        ...
        if (empty($this->options['confirmed_disabled'])) {
          ...
          $message = ( pll_current_language() == 'en' )?$this->options['confirmed_message']:$this->options['confirmed-ar_message'];
          $subject = ( pll_current_language() == 'en' )?$this->options['confirmed_subject']:$this->options['confirmed-ar_subject'];
    ...
    function unsubscribe() {
        ...
        if ($user->status == 'C') {
            ...
            if (empty($this->options['unsubscribed_disabled'])) {
                $subject = ( pll_current_language() == 'en' )?$this->options['unsubscribed_subject']:$this->options['unsubscribed-ar_subject'];
                $message = ( pll_current_language() == 'en' )?$this->options['unsubscribed_message']:$this->options['unsubscribed-ar_message'];
    ...
    function save_profile() {
        ...
        if (!$newsletter->is_email($_REQUEST['ne'])) {
            $profile_error = ( pll_current_language() == 'en' )?$this->options['profile_error']:$this->options['profile_error_ar']; 
            $user->alert = $profile_error;
            return $user;
        }
        ...
        if ($opt_in == 0 && $email_changed) {
            ...
            if (empty($this->options['confirmation_disabled'])) {
                if( pll_current_language() == 'en' ) {
                  ...
                  $message = $this->options['confirmation-ar_message'];
                  $subject = $this->options['confirmation-ar_subject'];
                }
                $res = $this->mail($user->email, $newsletter->replace($subject, $user), $newsletter->replace($message, $user));
                $profile_email_changed = ( pll_current_language() == 'en' )?$this->options['profile_email_changed']:  $this->options['profile_email_changed_ar'];
                $alert = $profile_email_changed;
            }
        }
    ...
    function show_message($key, $user, $alert = '', $email = null) {
        ...
        if( pll_current_language() == 'en' ) {
          ...
          if (!empty($this->options['page_ar'])) {
              $url = get_permalink($this->options['page_ar']);
              if( strpos($url, 'ar-newsletter') !== false ) {
                $url = str_replace('ar-newsletter', 'newsletter', $url);
              }
          }
        }
    ...
    function get_form_javascript() {
        ...
        $email_error = ( pll_current_language() == 'en' )?$options_profile['email_error']:$options_profile['email_error_ar']; 
        $profile_error = ( pll_current_language() == 'en' )?$options_profile['profile_error']:$options_profile['profile_error_ar']; 
        ...
        $buffer .= '        alert("' . addslashes($email_error) . '");' . "\n";
        $buffer .= '        return false;' . "\n";
        $buffer .= '    }' . "\n";
        ...
        $buffer .= '        alert("' . addslashes($profile_error) . '");' . "\n";
        $buffer .= '        return false;' . "\n";
        $buffer .= '    }' . "\n";
        $buffer .= '    }' . "\n";
    ...
    function get_subscription_form_html5($referrer = null, $action = null, $attrs = array()) {
        ...
        $buffer .= '<form method="post" id="newsletter-widget" action="' . esc_attr($action) . '" onsubmit="return newsletter_check(this)">' . "\n\n";
        ...
        if( pll_current_language() == 'en' ) {
          $email_text = $options_profile['email'];
        }
        else {
          $email_text = $options_profile['email_ar'];
        }
        
        $buffer .= '<label class="col-sm-10 col-sm-offset-2 inner-title">'. esc_html($email_text) .'</label>';
        $buffer .= '<div class="tnp-field tnp-field-email col-sm-6  col-sm-offset-2">';
        $buffer .= '<input class="tnp-email form-control" type="email" name="ne" required></div>';
        $buffer .= "\n";
        ...
        if (strpos($options_profile['subscribe'], 'http://') !== false) {
            ...
            if( pll_current_language() == 'en' ) {
              $subscripe_text = $options_profile['subscribe'];
            }
            else {
              $subscripe_text = $options_profile['subscribe_ar'];
            }
            $buffer .= '<input class="tnp-submit btn btn-primary col-sm-2" type="submit" value="' . esc_attr($subscripe_text) . '">' . "\n";
        }
    ...
    function get_profile_form_html5($user) {
        ...
        $email = (pll_current_language() == 'en') ? $options['email'] : $options['email_ar'];
        
        $buffer .= '<label class="col-sm-10 col-sm-offset-2 inner-title">'. esc_html($email) .'</label>';
        
        $buffer .= '<div class="tnp-field tnp-field-email col-sm-6  col-sm-offset-2">';
        $buffer .= '<input class="tnp-email form-control" type="text" name="ne" required value="' . esc_attr($user->email) . '">';
        $buffer .= "</div>\n";
        ...
        $save = (pll_current_language() == 'en') ? $options['save'] : $options['save_ar'];
        
        $buffer .= '<input class="tnp-submit btn btn-primary col-sm-2" type="submit" value="' . esc_attr($save) . '">';
        $buffer .= "</div>\n";
...
//    }
        ...
        if( pll_current_language() == 'en' ) {
          ...
          $message .= $module->options[$message_key . '_text_ar'];
        }
        ...
        $header =  '<section class="container">'
                    . '<div class="row">'
                    . '<div class="col-sm-offset-1 col-sm-10" id="internal-subscribe">'
                    . '<i class="fa fa-envelope-o basic-icon"></i>'
                    . '<div class="subscribe-text" style="text-align: center;width: 90%;display: block;margin: 0 auto;">';
        
        $footer .= '<div class="social-follow">'
                    ...
                    . '<a href="https://linkedin.com/company-beta/17946713/" target="_blank"><i class="fa fa-linkedin"></i></a>'
                    . '</div></div></div></section>';
