<?php
/**
 * Template Name: Edit Expert Thought
 *
 * @package egyptfoss
 */
if (!empty($_POST['action']) && $_POST['action'] == "edit_expert_thought") {
    
  $nonce = $_REQUEST['_wpnonce'];
  
  if (!wp_verify_nonce($nonce, 'edit-expert-thought')) {
    wp_redirect(home_url('/?status=403'));
    exit;
  }
  
  $isSaved = ef_edit_expert_thought_front_end();
  
  if($isSaved)
  {
    wp_redirect(get_permalink($isSaved));
    exit;
  }
...
$interests_meta = get_post_meta( $post_id, 'interest', true );

$interests      = array_map( function( $i ){ return get_term( $i )->name; }, $interests_meta);
          ...
          <div class="new-coupon-form">
            <form id="expert_thought_form" name="expert_thought_form" method="post" action="" enctype="multipart/form-data">
              <div class="required">
                <?php
                $ef_expert_thought_messages = get_query_var("ef_expert_thought_messages");
                $expert_thought_title = (isset($_POST['expert_thought_title'])) ? $_POST['expert_thought_title'] : $post_data->post_title;
                $expert_thought_description = (isset($_POST['expert_thought_description'])) ? $_POST['expert_thought_description'] : $post_data->post_content;
                $expert_thought_interests_posted = (isset($_POST['interest'])) ? $_POST['interest'] : $interests;
              ...
              <?php wp_nonce_field('edit-expert-thought'); ?>
              <input type="hidden" name="action" value="edit_expert_thought" />
            </form>
          </div>
        </div>
      </main><!-- #main -->
