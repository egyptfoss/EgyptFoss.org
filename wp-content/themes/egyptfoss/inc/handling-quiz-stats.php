 ...
 * @return void
 */
function ef_qmn_generate_stats_page()
{
 ...
 * @return void
 */
function ef_qmn_stats_overview_tab()
{
	global $mlwQuizMasterNext;
	$mlwQuizMasterNext->pluginHelper->register_stats_settings_tab(__("Quiz Taken Stats", 'quiz-master-next'), "ef_qmn_stats_overview_content");
}
//add_action("plugins_loaded", 'ef_qmn_stats_overview_tab');
 ...
 * @return void
 */
function ef_qmn_stats_overview_content()
{
  ...
  $numOfData = 7;
	$data = ef_qmn_load_stats($range, $numOfData);
	$labels = "";
	$value = "";
  switch($range)
  {
    ...
    case 'weekly':
      ...
      break;
		...
		case 'monthly':
			for ($i=($numOfData - 1); $i >=0; $i--) {
				$stat_date = date("Y-m", mktime(0, 0, 0, date("m")-$i, 1, date("Y")));
	...
	<div class="ef_metabox-holder">
		<div class="postbox">
			...
			<div>
				<canvas id="ef_graph_canvas"></canvas>
        <div class="text-center verticaltext_content">Number of Trials </div>
				<script>
					var graph_data = {
					...
					window.onload = function(){
						var graph_ctx = document.getElementById("ef_graph_canvas").getContext("2d");
 ...
 */
function ef_qmn_load_stats($type, $amount = 0) {
	$stats = array();
	switch ($type) {
		case 'daily':
			global $wpdb;
