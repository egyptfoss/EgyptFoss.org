...
class HistoryAction extends FormlessAction {
	...
	protected function getPageTitle() {
        ...
        //commented getDescription to hide viewpagelogs in History
	...
	function onView() {
		...
		$action = htmlspecialchars( wfScript() );
                $edited_action = ''; //action was $action edited to fix issue #660
		$out->addHTML(
			"<form action=\"$edited_action\" method=\"get\" id=\"mw-history-searchform\">" .
			Xml::fieldset(
...
class HistoryPager extends ReverseChronologicalPager {
	...
	function getStartBody() {
		...
		$this->getOutput()->wrapWikiMsg( "<div class='mw-history-legend'>\n$1\n</div>", 'histlegend' );
                /**
                 * Changed by Maisara Khedr to fix the link in history compare button
                 */
		$s = Html::openElement( 'form', array( 'action' => $this->getTitle()->getFullURL(),
	...
	function revLink( $rev ) {
		...
		if ( $rev->userCan( Revision::DELETED_TEXT, $this->getUser() ) ) {
			$link = Linker::linkKnown(
				...
				array( 
                                    'action' => 'view', /* Change by Maisara Khedr to fix history links */
                                    'oldid' => $rev->getId() 
                                )
			);
		} else {
