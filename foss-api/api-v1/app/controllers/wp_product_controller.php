<?php
 class WPProductController extends EgyptFOSSController {
  ...
  public function addProduct($request, $response, $args) {
    ...
    foreach(array("industry","type") as $oneValue){
    ...
    $multiValue = array("platform", "license");
  ...
  public function viewProduct($request, $response, $args) {
    ...
    if ($product['post_status'] == 'publish' || $product['post_status'] == 'pending' ){
      ...
      $type_name = '';
      if(isset($product_meta['type']))
      {
        $type_name = $term->getTerm($product_meta['type']);
      }
      
      $product_data['industry'] = array_key_exists('industry', $product_meta) ? ( ($args["lang"] == "ar" && !empty($industry_name['name_ar'])?html_entity_decode($industry_name['name_ar']):html_entity_decode($industry_name['name'])) ) : "";
      $product_data['type'] = array_key_exists('type', $product_meta) ? ( ($args["lang"] == "ar" && !empty($type_name['name_ar'])?html_entity_decode($type_name['name_ar']):html_entity_decode($type_name['name'])) ) : "";
      ...
      if (array_key_exists('platform', $product_meta) && !empty($product_meta['platform'])){
        ...
        foreach ($platform_ids as $platform_id) {
          ...
          $platform_name = array_merge($platform_name, array($platform['name']));
        }
      ...
      if (array_key_exists('license', $product_meta) && !empty($product_meta['license'])){
        ...
        foreach ($license_ids as $license_id) {
          ...
          $license_name = array_merge($license_name, array($license['name']));
        }
  ...
  public function editProduct($request, $response, $args) {
    ...
    $required_params = ['product_id','title','description','industry'];
    ...
    /*$terms_data = array("industry" => array($args['industry']),
    ...
    $terms_history_data = $product->updatePostTerms($args["product_id"], $terms_data);
