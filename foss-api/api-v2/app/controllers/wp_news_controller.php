...
class WPNewsController extends EgyptFOSSController {
   ...
   *   description="Get related news data according to the passed news id",
   *   @SWG\Parameter(name="token", in="header", required=false, type="string", description="User token needed to display related news of pending news   if exists"),
   *   @SWG\Parameter(name="newsId", in="path", required=false, type="integer", description="News ID to retrieve its related news <br/> <b>[Required]</b>"),
   *   @SWG\Response(response="200", description="List related news"),
   *   @SWG\Response(response="422", description="Validation Error"),
   *   @SWG\Response(response="404", description="News not found")
   * )
   */
    public function listRelatedNews($request, $response, $args) {
      
      if( empty( $args["newsId"] ) || $args["newsId"] == "{newsId}" )  {
        return $this->renderJson($response, 422, Messages::getErrorMessage("missingValue"));
      }
      ...
      if ( $news && ( $news->post_status == 'publish' || ( $news->post_status == 'pending' && !empty( $user ) && $news->post_author == $user->ID ) ) ) {
        $postmeta = new Postmeta();
        
        $lang = "ar";
        
        $lang_meta = $postmeta->getMetaValue( $post_id, 'language' );
        $category_id = $postmeta->getMetaValue( $post_id, 'news_category' );
        
        if( !empty( $lang_meta ) ) {
         $lang_serialized = $postmeta->maybe_unserialize( unserialize( $lang_meta ) );
         $lang = $lang_serialized['slug'];
        }
        
        $post = new Post();

        $relatedNews = $post->getRelatedNews( $post_id, $category_id, $lang );
        
        if ( empty( $relatedNews ) ) {
          return $this->renderJson($response, 422, Messages::getSuccessMessage("nothingToDisplay"));
        } 
        else {
          $news_result = array();
