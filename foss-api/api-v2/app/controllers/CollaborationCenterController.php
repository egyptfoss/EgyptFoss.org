...
public function ef_return_status_by_permission_api($isShared,$document_id, $space_id, $owner_id, $current_user_id)
{
 ...
 /**
  ...
  public function getDocument($request, $response, $args) {
    ...
    $document_revisions = $return_result->documentHistory()->orderBy("ID","desc");
    
    
    $revisions = array(); 
    
    foreach( $document_revisions->get() as $revision ) {
      ...
      $revisions[] = array(
        ...
        'added_by'      =>  $this->return_user_info_list( $revision->editor_id )
      );
    }
    
    $last_version_obj = $document_revisions->first();
  ...
  /**
   * @SWG\Get(
   *   path="/collaboration/revisions/{revision_id}",
   *   tags={"Collaboration Center"},
   *   summary="get revision of a document with a certain user",
   *   description="get revision of a document with a certain user",
   *   @SWG\Parameter(name="token", in="header", required=false, type="string", description="User token needed to get a document<br/> <b>[Required]</b>"),
   *   @SWG\Parameter(name="revision_id", in="path", required=false, type="string", description="revision_id <br/> <b>[Required]</b>"),
   *   @SWG\Response(response="200", description="get document"),
   *   @SWG\Response(response="422", description="Validation Error"),
   * )
   */
  
  public function getRevision($request, $response, $args) {
    ...
    {
      return $this->renderJson($response, 422, Messages::getErrorMessage("emptyValue", "revision_id"));
    }
    
    if(! is_numeric($args['revision_id']))
    {
      return $this->renderJson($response, 422, Messages::getErrorMessage("wrongValue", "revision_id"));
    }
    
    $user_id = $loggedin_user->user_id;
    $collab_center = new CollaborationCenterItemHistory();
    $collab_center_item = new CollaborationCenterItem();
    
    $revision = $collab_center->getRevisionByID( $args['revision_id'] );
    
    if(sizeof($revision) == 0){
      return $this->renderJson($response, 200, Messages::getSuccessMessage("nothingToDisplay"));
    }
    
    $isOwner = $collab_center_item->isMyItem( $user_id, $revision->item_ID );
    $isShared = $collab_center_item->isSharedItemByUser($user_id, $revision->item_ID);
