...
class Post extends BaseModel {
  ...
  public function updatePostTerms($post_id, $terms_data, $isCreate = false,$post_tax_names = array(), $no_serialize = false, $ignore_indsutry = false) {
    $term_ids = array();
    ...
    if ($isCreate == false) {
      ...
      if( $ignore_indsutry && ($key = array_search('industry', $ef_registered_taxonomies)) !== false) {
          unset($ef_registered_taxonomies[$key]);
      }
      global $foss_prefix;
  ...
  /**
   * Ger related news of certain news
   * 
   ...
   * @return type
   */
  public function getRelatedNews( $post_id, $category_id, $lang )
  {
    global $foss_prefix;
    ...
    $sql = "SELECT * FROM {$foss_prefix}posts as post
          join {$foss_prefix}term_relationships as rel on post.ID = rel.object_id
          join {$foss_prefix}term_taxonomy as tax on rel.term_taxonomy_id = tax.term_taxonomy_id
          join {$foss_prefix}terms as t on t.term_id = tax.term_id
          join {$foss_prefix}postmeta as pstmeta on pstmeta.post_id = post.ID
    ...
    $results =   self::getConnectionResolver()->connection()->select($sql);
