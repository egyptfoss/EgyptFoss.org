...
//    public $primaryKey  = 'term_id';
  ...
  public function loadTaxonomyByTaxonomyType($taxonomy, $take, $skip, $hasAr)
  {
      if($hasAr)
      {
        $result = Term::join('term_taxonomy','term_taxonomy.term_id','=','terms.term_id')
                  ...
                  ->select('terms.term_id','terms.name','terms.name_ar', 'terms.slug', 'term_taxonomy.parent');
        if($take != -1){
          $result->take($take);
        }
        if($skip != -1){
   ...
   * @return type
   */
  public static function get_term_child( $term_id, $taxonomy ) {
    global $foss_prefix;
    
    $sql =  " SELECT term.term_id"
          . " FROM {$foss_prefix}terms AS term "
          . " JOIN {$foss_prefix}term_taxonomy AS termtax ON term.term_id = termtax.term_id"
          . " WHERE termtax.parent = '{$term_id}' AND termtax.taxonomy = '{$taxonomy}'";
